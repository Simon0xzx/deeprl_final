...
def learn(env,
    ...
    target_q_tp1_val = q_func(obs_tp1_float, num_actions, scope="target_q_func", reuse=False)

    q_func_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='q_func')
    target_q_func_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='target_q_func')

    q_act_t_val = tf.reduce_sum(q_val * tf.one_hot(act_t_ph, num_actions), axis=1)
    y_t_val = rew_t_ph + (1 - done_mask_ph) * gamma * tf.reduce_max(target_q_tp1_val, axis=1)
    total_error = tf.reduce_mean(tf.losses.huber_loss(y_t_val, q_act_t_val))
    ...
    for t in itertools.count():
        ...
        if not model_initialized or np.random.rand(1) < epsilon:
            ...
            action = np.argmax(session.run(q_val, feed_dict={obs_t_ph:obs_input}))
        obs, reward, done, info = env.step(action)
        ...
        if (t > learning_starts and
            ...
            if num_param_updates % target_update_freq == 0:
                session.run(update_target_fn)
                train_loss = session.run(total_error, feed_dict={obs_t_ph:obs_t_batch, act_t_ph:act_t_batch, rew_t_ph:rew_t_batch, obs_tp1_ph:obs_tp1_batch, done_mask_ph:done_mask_batch})

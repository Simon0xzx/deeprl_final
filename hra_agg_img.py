...
def aggregator(images, q_values, num_actions, scope, reuse=False):
    with tf.variable_scope(scope, reuse=reuse):
        out = layers.flatten(images)
        out = tf.concat([out, q_values])
        out = layers.fully_connected(out, num_outputs=128, activation_fn=None)
        out = layers.fully_connected(out, num_outputs=128, activation_fn=None)
...
def learn(env,
    ...
    q_val, img_val = q_func(obs_t_float, num_actions, scope="q_func", reuse=False)
    q_food, q_avoid, q_fruit, q_eat = q_val
    target_val, target_img_val = q_func(obs_tp1_float, num_actions, scope="target_q_func", reuse=False)
    ...
    q_total = aggregator(img_val, q_all, num_actions, scope="q_agg", reuse=False)
    ...
    q_weight_y += (1 - done_mask_ph) * gamma * tf.reduce_max(target_q_total, axis=1)# - q_weight_val
    ...
    for t in itertools.count():
        ...
        if not model_initialized or np.random.rand(1) < epsilon:
            ...
            action = session.run(action_selected, feed_dict={obs_tp1_ph:obs_input}) # potential problem
